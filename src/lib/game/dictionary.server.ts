...
/**
 * Undoes `frontCode`/gzip/base64 from `scripts/generate-wordlist.mjs` back
 * into a flat, uppercased word `Set`.
 *
 * Kept synchronous (`gunzipSync`, not `DecompressionStream`) deliberately:
 * `getDictionary`/`isWord` are called from `validateMove` in the pure,
 * synchronous `engine.ts`, which is shared by every move-handling route —
 * making this async would have forced that whole call chain async for a
 * decode that only ever needs to happen once per Worker isolate anyway.
 * `node:zlib`'s sync API works both under Next.js on Node and, via
 * `nodejs_compat` (see `wrangler.toml`), on the deployed Cloudflare Worker.
 */
