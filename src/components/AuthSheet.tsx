...
/**
 * Icon sizes are tuned per glyph so they read as equal weight — each has a
 * different footprint inside its viewBox. The fixed-width box keeps the labels
 * on one left edge regardless.
 */
const PERKS = [
  {
    Icon: TrophyNavIcon,
...
/** Cheap client-side shape check. The real validation is the provider's. */
const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type Phase = "choose" | "sending" | "sent";

/**
 * The login wall. Mounted once in the root layout and shown automatically
 * whenever the session resolves without an account — playing requires one.
 *
 * Because it is the only way into the app it is deliberately non-dismissable:
 * no close button, no backdrop dismiss, no Escape, no guest escape hatch. It
 * also means the copy is fixed; there is no "which surface asked for this"
 * to explain, since the answer is always "all of it".
 *
 * Google is the primary path and a magic link is the fallback, both landing on
 * `/auth/callback`.
 */
export function AuthSheet() {
  ...
  const onGoogle = useCallback(async () => {
    ...
    setGoogleBusy(true);
    try {
      // On success this navigates away, so there is nothing to do after it.
  ...
  const onEmail = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        // Covers rate limiting as well as delivery refusals; the provider's own
        // message is not player-friendly enough to show raw.
