...
│   │   ├── _layout.tsx
│   │   ├── index.tsx      # Home / lobby        — Task 13
│   │   ├── history.tsx    # Match history        — Task 20
│   │   ├── leaderboard.tsx # Leaderboard          — Task 20
│   │   └── profile.tsx    # Profile + settings   — Task 19
│   ├── game/[code].tsx    # The board             — Task 16-18
│   ├── join.tsx           # Join by room code    — Task 14
│   ├── how-to-play.tsx    # Public, reachable without an account — Task 21
│   ├── legal/privacy.tsx  # Public, reachable without an account — Task 21
│   └── globals.css        # @import tokens.native.css; @tailwind directives
├── components/
│   ├── AppProviders.tsx   # Platform -> Theme -> Session -> Query -> (Auth, Task 9)
...
Every screen above is a placeholder until its numbered task lands — this task (7) only proves the routing tree, the token pipeline and the provider stack mount together. `metro.config.js` watches the monorepo root and widens `nodeModulesPaths` so the shared packages' TypeScript source resolves the same way `transpilePackages` makes it resolve on the web. `apps/mobile/tsconfig.json` does **not** extend the shared `tsconfig.base.json` — Expo's own base sets compiler options (`jsx: "react-native"`, Metro-specific resolution) that would fight a layered config, so this is the one workspace with its own compiler contract.
